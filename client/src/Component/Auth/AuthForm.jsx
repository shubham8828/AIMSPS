  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isLogin) {
        ...
        await axios
          .post("https://aimsps-server.vercel.app/api/login", { email, password })
          .then((res) => {
            ...
            setFormData({ name: "", email: "", address: "", password: "", phone: "", image: defaultProfile });
            setLoading(false);
            navigate('/');
          })
          .catch((error) => {
        ...
        await axios
          .post("https://aimsps-server.vercel.app/api/register", formData)
          .then((res) => {
            localStorage.setItem('email', res.data.user.email);
            localStorage.setItem('token', res.data.token);
            setToken(res.data.token);
